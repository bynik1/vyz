...
public:
  ...
  void move_once(float &px, float &py, int &pvec_x, int &pvec_y) {//переджижение одной точке на координату один
    if (vec_x == 0 || vec_y == 0)
      return;
    if (px == 0)
      pvec_x = 1;
  ...
  void move_cent_once(float &x, float &y, float &x_c, float &y_c) {
    ...
    y = y_c + (dx - x_c) * sin(M_PI / 180) + (dy - y_c) * cos(M_PI / 180);//x y наша точка которую надо повернуть вокруг точки x_c и y_c 
  }//вращение вокруг своей оси

protected://наследуются другим
  int real;
  int color;
  float x, y;
  int vec_x, vec_y;
