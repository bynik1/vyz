...
void draw_trian_lt(Field &f, Triangle &trian) {//движение
  for (int x = 0; x < 2000; x++) {
    delay(DELAY_UMSHORT);
    cleardevice();
...
void draw_trian_urself_lt(Field &f, Triangle &trian) {//вокругн своей оси
  for (int x = 0; x < 2000; x++) {
    delay(DELAY_UMSHORT);
    cleardevice();
...
void draw_trian_lt_cp(Field &f, Triangle &trian) {//отскакивание центром
  for (int x = 0; x < 2000; x++) {
    delay(DELAY_UMSHORT);
    cleardevice();
...
/* ELLIPSE */ //нельзя крутить вокруг центра
