...
class String : public StrType {
    friend std::ostream& operator<<(std::ostream& os, String& s);//?

    friend bool operator<(const String& s1, const String& s2);//зачем все эти операторы
...
public:
    String() : s(nullptr), _size(0)//что значит запись
    {
    }

    String(const char* _s) : s(new char[strlen(_s) + 1]), _size(strlen(_s))//?
    {
        strcpy(s, _s);
    }
    ...
    String(String&& _s)
        : s(new char[strlen(_s.s) + 1]), _size(std::move(_s._size))
    {
        s = std::move(_s.s);
    }
...
class CharVector : public StrType {
