...
void tSquare(){
	std::cout << "square line is 0"<< endl;
}
...
void tSquare(){
	...
	std::cout << "square triangle is "<< S << endl;
}
...
public:
  ...
  void set_y4(int y4) { this->y4 = y4; }
	...
	void tSquare(){
		...
		b = sqrt((x2 - x3)*(x2 - x3) + (y2-y3)*(y2-y3));
  		S = a * b;
		std::cout << "square rectangle is " << S << endl;
	}
...
public:
  ...
  void set_rad(int rad) { this->rad = rad; }
	...
	void tSquare(){
		double S;
		S = 3.14*rad*rad;
		std::cout << "square line is " << S << endl;
	}
...
public:
  ...
  int get_y_rad() { return this->y_rad; }
	...
	void tSquare(){
		double S;
		S = 3.14*(x_rad*y_rad);
		std::cout << "square ellipse is " << S << endl;
	}
...
public:
  ...
  int get_y3() { return this->y3; }
	...
	void tSquare(){
	double S, P, a, b, c;
  double dx = x, dx2=x2, dx3=x3, dx4=x4, dy=y, dy2=y2, dy3=y3, dy4=y4;
	a = sqrt((dx - dx2)*(dx - dx2) + (dy-dy2)*(dy-dy2));
	b = sqrt((dx2 - dx3)*(dx2 - dx3) + (dy2-dy3)*(dy2-dy3));
	c = sqrt((dx - dx3)*(dx - dx3) + (dy-dy3)*(dy-dy3));
	P = a + b + c;
	S = 2 * sqrt(P*(P-a)*(P-b)*(P-c));
	std::cout << "square dimond is " << S << endl;
	}
