...
int main(int argc, char *argv[]) {
    ...
    MPI_Dims_create(commsize, 2, dims); // Создание размеров сетки
    ...
    int rankx = coords[0]; // Координата x текущего процессаё
    ...
    for (;;) { // Начало бесконечного цикла
        ...
        // Обмен граничными данными с соседними процессами
        thalo -= MPI_Wtime();
